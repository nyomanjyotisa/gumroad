...
const PaymentElementBody = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      // Multi-seller (and other reusable) carts charge each seller off a saved payment method, so we
      // turn the Payment Element card into a reusable pm via the existing SetupIntent flow. The
      // SCA confirmation (confirmCardIfNeeded -> confirmCardSetup) acts on the SetupIntent's
...
export const PaymentElementCardContent = () => {
  ...
  // Reusable carts (multi-seller, etc.) set up a saved pm; everything else charges the total now.
