...
describe UrlRedirectPresenter do
  ...
  describe "#download_attributes" do
    it "returns all necessary attributes for the download page" do
      allow(Aws::S3::Resource).to receive(:new).and_return(double(bucket: double(object: double(content_length: 1, public_url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/4768692737035/bb69798a4a694e19a0976390a7e40e6b/original/chapter1.srt"))))
    ...
    it "includes thumbnail_url for product files that have thumbnails" do
      ...
      content_items = instance.download_attributes[:content_items]
      expect(content_items.length).to eq(1)
      expect(content_items.first[:thumbnail_url]).to eq("https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{file.thumbnail_variant.key}")
    end
  end
