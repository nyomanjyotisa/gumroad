...
describe Workflow::SaveInstallmentsService do
  ...
  describe "#process" do
    ...
    def process_and_perform_assertions_for_created_installments
      params[:installments] = [default_installment_params.merge(id: SecureRandom.uuid, files: [{ external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/file1.mp4", position: 1, stream_only: true, subtitle_files: [{ url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub1-uuid/en.srt", language: "English" }, { url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub2-uuid/es.srt", language: "Spanish" }] }, { external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/file.pdf", position: 2, stream_only: false, subtitle_files: [] }])]
      ...
      expect(file1.url).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/file1.mp4")
      expect(file1.position).to eq(1)
      expect(file1.stream_only).to be(true)
      expect(file1.filegroup).to eq("video")
      expect(file1.subtitle_files.alive.pluck(:language, :url)).to eq([["English", "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub1-uuid/en.srt"], ["Spanish", "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub2-uuid/es.srt"]])
      file2 = installment.alive_product_files.last
      expect(file2.url).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/file.pdf")
    ...
    def process_and_perform_assertions_for_updated_installments
      ...
      video = create(:product_file, installment:, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/video.mp", link: nil, position: 1, stream_only: true)
      video.subtitle_files << create(:subtitle_file, product_file: video, language: "English", url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub1-uuid/en.srt")
      video.subtitle_files << create(:subtitle_file, product_file: video, language: "Spanish", url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub2-uuid/es.srt")
      pdf = create(:product_file, installment:, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/doc.pdf", link: nil, position: 2, stream_only: false)
      audio = create(:product_file, installment:, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/audio.mp3", link: nil, position: 3, stream_only: false)
      params[:installments] = [default_installment_params.merge(id: installment.external_id, name: "Installment 1 (edited)", message: "Updated message", time_duration: 2, time_period: "day", files: [{ external_id: video.external_id, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/video.mp4", position: 1, stream_only: true, subtitle_files: [{ url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub1-uuid/en.srt", language: "English" }, { url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub2-uuid/es.srt", language: "German" }] }, { external_id: pdf.external_id, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/doc.pdf", position: 2, stream_only: false, subtitle_files: [] }, { external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/book.epub", position: 3, stream_only: false, subtitle_files: [] }])]
      service = described_class.new(seller:, params:, workflow:, preview_email_recipient:)
      ...
      file1 = installment.alive_product_files.first
      expect(file1.url).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/video.mp4")
      expect(file1.position).to eq(1)
      expect(file1.stream_only).to be(true)
      expect(file1.filegroup).to eq("video")
      expect(file1.subtitle_files.alive.pluck(:language, :url)).to eq([["English", "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub1-uuid/en.srt"], ["German", "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/sub2-uuid/es.srt"]])
      file2 = installment.alive_product_files.second
      expect(file2.url).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/doc.pdf")
      expect(file2.position).to eq(2)
      expect(file2.stream_only).to be(false)
      expect(file2.filegroup).to eq("document")
      file3 = installment.alive_product_files.last
      expect(file3.url).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/book.epub")
    ...
    it "deletes installments that are missing from the params" do
      installment = create(:installment, workflow:)
      attached_file = create(:product_file, installment:, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/doc.pdf", link: nil)
      service = described_class.new(seller:, params:, workflow:, preview_email_recipient:)
      expect { service.process }.to change { workflow.installments.alive.count }.by(-1)
