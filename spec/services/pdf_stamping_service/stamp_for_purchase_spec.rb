  ...
  describe ".perform!" do
    ...
    context "with stampable PDFs" do
      ...
      it "creates stamp_pdf and updates url_redirect" do
        ...
        expect(stamped_pdf.url).to match(/s3.ap-southeast-2.amazonaws.com/)
        expect(url_redirect.reload.is_done_pdf_stamping?).to eq(true)
      end

      context "with encrypted stampable PDFs" do
        let!(:product_file_two) { create(:readable_document, pdf_stamp_enabled: true, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/encrypted-GameFu.pdf") }
        let!(:product_file_three) { create(:readable_document, pdf_stamp_enabled: true) }
        let!(:product_file_four) { create(:readable_document, pdf_stamp_enabled: false, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/encrypted-GameFu.pdf") }
