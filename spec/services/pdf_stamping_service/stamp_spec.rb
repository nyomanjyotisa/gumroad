...
describe PdfStampingService::Stamp do
  describe ".can_stamp_file?" do
    context "with readable PDF" do
      let(:pdf) { create(:readable_document, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/billion-dollar-company-chapter-0.pdf") }
    ...
    context "with encrypted PDF" do
      let(:pdf) { create(:readable_document, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/encrypted-GameFu.pdf") }
  ...
  describe ".perform!" do
    let(:pdf_url) { "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/billion-dollar-company-chapter-0.pdf" }
    ...
    context "when applying the watermark fails" do
      context "when the PDF is encrypted" do
        let(:pdf_url) { "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/encrypted-GameFu.pdf" }
