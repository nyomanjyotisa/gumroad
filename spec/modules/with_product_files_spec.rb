...
describe WithProductFiles do
  ...
  describe "#save_files!" do
    context "when called on a Link record" do
      ...
      it "enqueues a `PdfUnstampableNotifierJob` job when new stampable files are added" do
        ...
        product_files_params << { external_id: SecureRandom.uuid, pdf_stamp_enabled: false, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/23b2d41ac63a40b5afa1a99bf38a0982/original/nyt.pdf" }
        product_files_params << { external_id: SecureRandom.uuid, pdf_stamp_enabled: true, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/manual.pdf" }
      ...
      it "does not enqueue a `PdfUnstampableNotifierJob` job when new non-stampable files are added" do
        product_files_params = product.product_files.each_with_object([]) { |file, params| params << { external_id: file.external_id, url: file.url } }
        product_files_params << { external_id: SecureRandom.uuid, pdf_stamp_enabled: false, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/manual.pdf" }
      ...
      it "does not enqueue a `PdfUnstampableNotifierJob` job for a non-product resource" do
        ...
        expect do
          post.save_files!([{ external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/manual.pdf" }])
      ...
      it "sets content_updated_at when new files are added" do
        product_files_params = product.product_files.each_with_object([]) { |file, acc| acc << { external_id: file.external_id, url: file.url } }
        product_files_params << { external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/pic.jpg" }
        product_files_params << { external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/2/original/chapter2.mp4" }
      ...
      it "updates file attributes" do
        ...
        product_files_params << { external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/pic.jpg", size: 2, position: 0 }
      ...
      it "preserves folder_id on file if folder is deleted" do
        ...
        file_1 = create(:product_file, link: product, description: "pencil", url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/pencil.png", folder_id: folder_1.id)
        file_2 = create(:product_file, link: product, description: "manual", url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/manual.pdf", folder_id: folder_2.id)
    ...
    context "when called on an Installment record" do
      ...
      it "does not enqueue a `PdfUnstampableNotifierJob` job when a new file is added" do
        expect do
          installment.save_files!([{ external_id: SecureRandom.uuid, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/pic.jpg" }])
  ...
  describe "#has_been_transcoded?" do
    ...
    it "indicates that it hasn't been transcoded if none of the transcoded videos are completed" do
      ...
      file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/2/original/chapter2.mp4")
