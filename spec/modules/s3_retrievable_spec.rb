...
describe "S3Retrievable" do
  ...
  subject(:s3_retrievable_object) do
    model.new.tap do |test_class|
      test_class.url = "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/nyt.pdf"
    end
  end
  ...
  describe "#unique_url_identifier" do
    it "returns url as an identifier" do
      expect(s3_retrievable_object.unique_url_identifier).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/nyt.pdf")
    end

    context "when it has an s3 guid" do
      before do
        s3_retrievable_object.url = "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/23b2d41ac63a40b5afa1a99bf38a0982/original/nyt.pdf"
      end
  ...
  describe "#download_original" do
    ...
    it "raises a descriptive exception if the S3 object doesn't exist" do
      record = model.create!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/missing.txt")
  ...
  describe "#s3_url" do
    it "returns s3 url value" do
      expect(s3_retrievable_object.s3_url).to eq("https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/nyt.pdf")
    end
  ...
  describe "#restore_deleted_s3_object!" do
    context "when the versioned object exists" do
      let!(:record) { model.create!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/#{SecureRandom.hex}") }
  ...
  describe "#confirm_s3_key!" do
    it "updates the url if possible" do
      ...
      record = model.create!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/#{s3_directory}/incorrect-file-name.pdf")
    ...
    it "does nothing if the file exists on S3" do
      previous_url = "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/sample.mov"
  ...
  describe ".with_s3_key" do
    it "only includes s3 files matching the s3 key" do
      foo = model.create!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/foo.pdf")
      foo2 = model.create!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/foo.pdf")
      other = model.create!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/other.pdf")
      model.create!(url: "https://example.com")
