  ...
  describe "#preview_thumbnail_url" do
    context "when a thumbnail is attached" do
      it "returns a valid representation URL" do
        ...
        expect(video_file.thumbnail_url).to eq("https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{video_file.thumbnail.key}")

        video_file.thumbnail.variant(:preview).processed
        expect(video_file.thumbnail_url).to eq("https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{video_file.thumbnail.variant(:preview).key}")
      end
    end
