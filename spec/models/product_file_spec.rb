...
describe ProductFile do
  ...
  describe "#has_alive_duplicate_files?" do
    let!(:file_1) { create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/some-file.pdf") }
    let!(:file_2) { create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/some-file.pdf") }
  ...
  describe "file group and analyze" do
    ...
    it "preserves s3 key for files containing percent and ampersand in filename" do
      product_file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/test file %26 & ) %29.txt")
    ...
    it "creates the product file with filetype set to lowercase" do
      product_file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/fc34ee33bae54181badd048c71209d24/original/sample.PDF")
  ...
  describe "s3 properties" do
    before do
      @product_file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/files/43a5363194e74e9ee75b6203eaea6705/original/black/white.mp4")
    end
    ...
    it "works as expected for files without an extension" do
      @product_file.update!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/files/43a5363194e74e9ee75b6203eaea6705/original/black/white")
  ...
  describe "#hls_playlist" do
    before do
      ...
      @file_1 = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/2/original/chapter 2.mp4", is_transcoded_for_hls: true)
    ...
    it "escapes the user-provided filename even if the user has changed the filename since the video was transcoded" do
      @file_1.update!(url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/2/original/chapter_2_no_spaces.mp4")
      travel_to(Date.parse("2015-03-13")) do
        hls_playlist = @file_1.hls_playlist
        url = "#EXTM3U\n#EXT-X-STREAM-INF:PROGRAM-ID=1,RESOLUTION=854x480,CODECS=\"avc1.4d001f,mp4a.40.2\",BANDWIDTH=1191000\n"
    ...
    it "escapes the filename in legacy S3 attachments" do
      file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/0000134abcdefghhijkl354sfdg/chapter 2.mp4", is_transcoded_for_hls: true)
    ...
    it "escapes the newlines in the filename" do
      file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/12345/abcd12345/original/YouTube + Marketing Is Powerful\n.mp4", is_transcoded_for_hls: true)
      @multifile_product.product_files << file
      @transcoded_video = create(:transcoded_video, link: @multifile_product, streamable: file, original_video_key: file.s3_key,
  ...
  describe "#subtitle_files_for_mobile" do
    ...
    context "when associated alive subtitle files exist" do
      let(:english_srt_url) { "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/english.srt" }
      let(:french_srt_url) { "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachment/french.srt" }
  ...
  describe "mobile" do
    before do
      @file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/2/original/chapter 2.mp4", is_transcoded_for_hls: true)
    end
  ...
  describe "#display_extension" do
    ...
    it "returns empty string for s3 files without an extension" do
      product_file = create(:product_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/files/43a5363194e74e9ee75b6203eaea6705/original/black/white")
  ...
  describe "thumbnail" do
    describe "validations" do
      ...
      it "is valid if the attached thumbnail is an image with size smaller than 5 MB" do
        ...
        expect(product_file.thumbnail_variant.url).to match("https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{product_file.thumbnail_variant.key}")
      end
    end
    ...
    describe "#thumbnail_url" do
      ...
      it "returns the CDN URL if thumbnail exists" do
        stub_const("CDN_URL_MAP", { "https://gumroad-specs.s3.ap-southeast-2.amazonaws.com" => "https://public-files.gumroad.com" })
      ...
      it "returns the original thumbnail URL when the thumbnail variant processing fails" do
        ...
        expect(product_file.thumbnail_url).to match("https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{product_file.thumbnail.key}")
      end
    end
    ...
    describe "#reset_moderated_by_iffy_flag" do
      ...
      context "when an image product file is created" do
        it "resets moderated_by_iffy flag on the associated product" do
          expect do
            create(:product_file, link: product, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/specs/kFDzu.png")
