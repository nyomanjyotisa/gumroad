...
describe("Download Page – Rich Text Editor Content", type: :system, js: true) do
  ...
  context "digital product" do
    ...
    it "shows inline preview for video file embeds" do
      subtitle_file = create(:subtitle_file, product_file: @video_file, url: "https://s3.ap-southeast-2.amazonaws.com/gumroad-specs/attachments/6505430906858/ba3afa0e200b414caa6fe8b0be05ae20/original/sample.srt")
      ...
      within(find_embed(name: "Video file")) do
        expect(page).to have_selector("img[src='https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{@video_file.thumbnail_variant.key}']")
        click_on "Watch"
        expect(page).to_not have_button("Watch")
        expect(page).to_not have_selector("img[src='https://gumroad-specs.s3.ap-southeast-2.amazonaws.com/#{@video_file.thumbnail_variant.key}']")
        expect(page).to have_selector("[aria-label='Video Player']")
        expect(page).to have_button("Pause")
